...
- **Video clips (Open, Video clip):** up to 6 seconds, read at 8 frames a second (6 on a phone); each frame gets depth, eased toward the frames before it so it does not flicker. The first frame sets up the scene (finder, floors, sky); recorded videos step through the clip while the move and effect play (once, or out and back with Loop). Clip frame on the Move tab picks the still. Exports rebuild each clip frame, so they take a while (about a minute for 4 seconds on a PC).
