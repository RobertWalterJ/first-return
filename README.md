...
- **Measure (ruler button on the picture):** single-view metrology, not depth (which one photo squeezes at a distance). Drag the yellow eye-level tag to eye height (where a standing person's eyes would be); with the camera 1.5 m up (the Camera chip changes it), tap where something meets the ground, then its top for its height or another spot on the ground for a distance. Set length corrects everything from something you know. Scans measure directly in their own units. Measurements show on the picture and in saved pictures and videos.
