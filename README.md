...
- **Scan from video (Open, Scan from video):** walk slowly around something for 10 to 30 seconds and the phone builds a real multi-view 3D splat scene, with no server or training loop: about 2.5 frames a second (up to 40), depth for each, FAST corners with BRIEF descriptors matched between neighbouring frames and lifted into 3D, the camera's move between frames solved as a similarity (Horn's method inside RANSAC), then every frame fused into one voxel grid (voxels seen from only one frame are dropped as floaters) and turned into splats. It opens like any scan. Not a trained splat, but truly three-dimensional.
