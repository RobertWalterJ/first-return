...
function renderMeasureBar(){
  ...
  const how = S.mCorr!==1 ? 'Corrected by your known length.' : S.scan ? 'Scan units, usually metres.' : `The camera is taken as ${S.camH} m above the ground, about where a phone is held (Camera changes it). Expect some error; Set length makes it better.`;
  // the steps, the one to do now highlighted
  const steps = S.scan
    ? ['Tap the first point.', 'Tap the second point.']
    : ["Drag the yellow line to the camera's height.", 'Tap where something meets the ground.', 'Tap its top for its height, or another spot on the ground for a distance.'];
  const now = S.scan ? S.mPts.length : (S.mPts.length ? 2 : S.hzMoved || S.mLines.length ? 1 : 0);
  const spoken = steps[now] + (now===0 && !S.scan ? " That is the height the photo was taken from. Standing people's eyes sit on it; on a flat street, it is where the road would meet the sky." : '') + ' ' + how;
  bar.innerHTML = `<h3>Measure<span class="sp">${sayBtn(spoken)}</span></h3>
    <ol class="msteps">${steps.map((t,i)=>`<li class="${i===now?'now':i<now?'done':''}"><span>${t}${i===0 && now===0 && !S.scan ? `<small>That is the height the photo was taken from. Standing people's eyes sit on it; on a flat street, it is where the road would meet the sky.</small>` : ''}</span></li>`).join('')}</ol>
...
function renderMeasures(){
  ...
  if (S.measure && !S.scan && S.photo){ const far=400, hz=horizonV(), l=P(unproject(-0.2,hz,far)), r=P(unproject(1.2,hz,far));
    if (l && r){ const hy = l[1] + (r[1]-l[1])*((ox+14-l[0])/((r[0]-l[0])||1));
      svg += `<line x1="${l[0]}" y1="${l[1]}" x2="${r[0]}" y2="${r[1]}" class="mhz"/><g class="mtag"><rect x="${ox+6}" y="${hy-13}" width="120" height="26" rx="13"/><text x="${ox+60}" y="${hy+5}">&#x21D5; camera height</text></g>`; } }
...
cv.addEventListener('pointerdown', e=>{ if (!S.measure || S.scan || !S.photo || !S.P) return;
  ...
  if (s && x < 136 && Math.abs(y-s[1]) < 26){ hzDrag = {y0:e.clientY, v0:horizonV()}; e.stopImmediatePropagation(); try{ cv.setPointerCapture(e.pointerId); }catch(err){} } }, true);
