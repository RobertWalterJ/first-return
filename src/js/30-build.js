...
function buildCloud(cfg){
  ...
  const floorN = cfg.floor > 0 ? Math.round(cfg.floor * Math.min(90000, 9000 + target*0.08)) : 0;
  target = Math.max(1000, Math.min(target, (cfg.cap||MAXPTS) - floorN - 2000));
  const cap = Math.round(target*1.08) + floorN + 4000;
  ...
  let fx=0.5, fy=0.5; { let sx=0,sy=0,c=0; for (let i=0;i<map.length;i+=3) if (map[i]>=0){ sx+=i%D.w; sy+=(i/D.w)|0; c++; } if (c){ fx=sx/c/D.w; fy=sy/c/D.h; } }
...
function floorPlane3D(live){
  ...
  if (S.plane && G){
    const rnd = seeded(5), rows=[];
    for (let k=0;k<6000 && rows.length<1500;k++){ const i=(rnd()*G.length)|0; if (G[i]!==1) continue;
    ...
    if (rows.length > 200){
      ...
      const D0=det(A);
      if (Math.abs(D0)>1e-9){ const col=i=>A.map((r,k)=>r.map((v,j)=>j===i?B[k]:v));
        const a=det(col(0))/D0, c=det(col(1))/D0, b=det(col(2))/D0;
        if (Math.abs(a)<0.5 && Math.abs(c)<1.2) return {a,c,b}; }
    }
  }
  return {a:0, c:0, b:lowest};
}
