...
function build(){
  ...
  if (S.autoLight && !S.scan){
    ...
    if (P.light && !S.isSample){ notice((S.noticeText ? S.noticeText+' ' : '') +(P.light===2 ? 'The subject was in shadow, so its light is evened out (Adjust, Shadows).' : 'The subject was dark, so its light is lifted (Adjust, Shadows).')); }
